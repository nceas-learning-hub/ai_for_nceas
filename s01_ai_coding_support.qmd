...
GitHub Copilot is an AI-powered code completion tool that helps you write code faster and with fewer errors. It is available as a plugin for Visual Studio Code, and it can also be used in RStudio.  Copilot is a large language model, related to ChatGPT, trained on GitHub and other codebases, and it can generate code completions based on the context of your code.  In this lesson, we will configure RStudio to work with GitHub Copilot and practice techniques to use Copilot to assist with coding tasks.

If you have an [education GitHub account](https://github.com/education) (e.g., an .edu email address that is associated with your GitHub account), you should be able to get free access to Copilot.  Otherwise you may need to pay for a subscription.  See [more Copilot requirements here](https://docs.posit.co/ide/user/ide/guide/tools/copilot.html).
...
Here's what Copilot suggested for me writing this lesson.  Note that it recognized data in previous code chunks, and when I started writing a comment to test, it assumed I wanted to use that data.
...
Does Copilot's suggestion work for you?  If not take a moment to fix it and make it work.  Some observations on the code presented here:

* I would probably avoid overwriting the local value of `x` and `y` in the function, as it could be confusing.
